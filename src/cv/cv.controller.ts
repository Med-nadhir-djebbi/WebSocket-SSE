...
export class CvController {
  ...
  create(@Body() createCvDto: CreateCvDto, @getUser() user: { id: number }) {
  ...
  @Get(':id')
  @UseGuards(AuthGuard('jwt'))
  findOne(@Param('id') id: string, @getUser() user: { id: number }) {
    return this.cvService.findOne(+id, user.id);
  }

  @Patch(':id')
  @UseGuards(AuthGuard('jwt')) 
  update(@Param('id') id: string, @Body() updateCvDto: UpdateCvDto, @getUser() user: { id: number }) {
